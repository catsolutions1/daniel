...
func calculate_target_position(room_center: Vector2, room_size: Vector2) -> Vector2:
	...
	if x_margin <= 0:
		...
		return_position.x = clamp(LevelManager.player.position.x, left_limit, right_limit)
	...
	if y_margin <= 0:
		...
		return_position.y = clamp(LevelManager.player.position.y, top_limit, bottom_limit)
